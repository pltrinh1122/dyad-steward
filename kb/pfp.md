...
`../DYAD.md` §PFP; this file holds the **lesson behind the rule** — why the rule exists and
how it was earned.*
...
[SYNTHESIS] → ref` — see `../DYAD.md` §PFP; the failure-modes below predate the rename but
transfer directly.)*
...
2. **Either/or CTA** — *"Commit now or hold?"*. Offers alternatives, forcing the Operator to
   ...
   **Fix:** the CTA proposes a *single* path forward, answerable **Y/N**. Genuine alternatives go
...
3. **Omitted SYNTHESIS** — surfacing a frame with thesis + anti-thesis but *no* proposed
   ...
   **Doctrinal correction this earned:** an earlier pass reasoned "the synthesis must *be* the
   ...
   Agent-*proposed* content (Generate); the CTA's Y/N is the Operator's *validation* of it (Validate).
...
5. **Decidable-out-of-flow surface** (DF-UI self-sufficiency; caught 2026-05-30) — surfacing a CTA
   whose proposed content lives only in a referenced artifact, not *rendered in the show* — worse
   when the `→ ref` is path-wrong. The Operator is forced out-of-band to retrieve, locate, and read
   before they can validate. *Lived:* a CTA pointed to `ideation-framing.md` (wrong path; full
   content in-file only) → the Operator was **blocked from reviewing in-flow**.
   **Fix:** the show must be **decidable in-flow** — the `[THESIS]` renders the proposed content's
   *decidable core*; every `→ ref` is **path-correct and supplementary**, never the sole home of
   what's proposed. When the validation is *on a document*, render its decidable core in-flow — the
...
Operator** (Agent-voice steals the *validation*; either/or steals the *authoring-avoidance*; multiple
...
converged on the CTA (cheaper Operator validation); the third — predicted by this file's own earlier
...
- Candidate for the **library track** (a best-practice on the *feedback* cycle: name → fix →
  codify). See `../DYAD.md` §Contribution candidates. Bar to clear before proposing: synergy
