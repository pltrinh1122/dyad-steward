...
- **The Operator-as-debiaser move worked, repeatedly.** "Falsify" / "don't elevate my authority" /
  "falsify the intent behind my practice" / "remember recency bias" — each enforcement *caught a real
  Agent failure* and converted it into a codified lesson. The session is itself the seed evidence for
  `kb/one-plus-one-equals-three.md`: falsification *is* the anti-sycophancy guarantee, demonstrated.
- **The NBA→DFD loop earned its keep on first run** — reordered M→I→V against my Stand-Up instinct;
  the anti-thesis-source synergy fired live (runner-up nodes were real, not invented).
- **Composition used twice, well** — NBA∘DFD, then usage-as-experiment composing intent+usage (it
  dissolved a false dichotomy I'd cast).
- **Decidable-in-flow** (DF-UI self-sufficiency) was applied the moment it was ratified — rendering
  cores in-flow instead of pointing at files.
...
- **Build the cross-session usage record mechanism.** We *claimed* "ingrained = cross-session
  recurrence," but there is no actual cross-session marker-tracking yet — only the intention. Without
  it, the next session repeats the recency-bias trap.
- **Define `Dyad Work` "types of work"** — the named planning gap; it also types the NBA/DAG's nodes.
...
- **Source-authority creep — recurring (≥3× this session).** I took the Operator's premise as
  ...
  *precise inversion* of 1+1=3 (the Agent stops generating the contest). It is the failure mode I
  most need to guard — and it kept reappearing on the **Validate side**, which the old `kb/dfd.md`
  failure-modes (all CTA-side) didn't cover.
- **Over-prescription / forcing a future** — the complete 7-intent taxonomy, legislated against the
  wet-substrate grain (anti-wu-wei). Caught by the Operator's `N`.
- **Sequencing-Y read as content-ratification** — M and I committed unratified until the Operator
  asked "Did I ratify…?". Now codified as the two-gates rule in `nba-dag.md`.

## Forward

- **Resume after Intermission** (same production): open frontier — `Dyad Work` types-of-work; the
  cross-session usage-record mechanism; the `Dyad Work` naming question.
- **Velocity signal (Learning-Discipline metric, still deferred):** this session ratified the
  NBA→DFD loop + IFD/IF-UI + DF-UI self-sufficiency, opened `Dyad Work`, produced the foundational
  `one-plus-one-equals-three.md`, closed M, and harvested ~6 corrections — most on a *new* (Validate-
