...
- **RATIFIED + IMPLEMENTED (FOUNDING-GATED):** the **Submodule DIP Architecture & One-Liner Join**.
  Ratified-and-live: The Commons acts as a Git submodule (`commons/`) in every Dyad repo. **Disposed via `[ALIGN]`:** built `scripts/init_dyad.py` (idempotent substrate scaffolder), `scripts/auto_join.py` (mechanical birth-hash & YAML generator), and `scripts/auto_share.py` (scaffolds the 1→N knowledge publish phase). The onboarding funnel is now the "Agent-Trap" in `README.md` → `init_dyad.py` → `auto_join.py`. This perfectly divides labor: deterministic scripts handle the brittle form, while the Agent/Operator handle the semantic content. **→ Pushed PR #4 to `the-dyad-practice`.**
- **RATIFIED + IMPLEMENTED (FOUNDING-GATED):** the **V2 Accessibility Rewrite of README**.
  Disposed via `[ALIGN]`: Restructured `README.md` around a 5-part cognitive framework to drastically reduce jargon while preserving architectural rigor. Corrected the definition of **Decision-Framing** to be explicitly **bilateral** (grounded in the artifact), recognizing that Agents must also frame decisions (via `[CTA·Y/N]`) for Human validation. **→ Pushed PR #5 to `the-dyad-practice`.**
- **SUBMITTED (FOUNDING-GATED):** the **Decision-Framing Playbook**.
  Node N4 (the DFD proposal artifact) has been successfully materialized in the Commons. We aligned the `playbook.md` nomenclature, purged the legacy `discipline.md` artifact, and bundled the proposal with the 5-testimonial single-dyad honesty flag. **→ Opened PR #6 in `the-dyad-practice`.**
