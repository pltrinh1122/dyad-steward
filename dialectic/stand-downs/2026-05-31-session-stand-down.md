...
- **RATIFIED + IMPLEMENTED (FOUNDING-GATED):** the **V2 Accessibility Rewrite of README**.
  Disposed via `[ALIGN]`: Restructured `README.md` around a 5-part cognitive framework to drastically reduce jargon while preserving architectural rigor. Corrected the definition of **Proposal-Framing** to be explicitly **bilateral** (grounded in the artifact), recognizing that Agents must also frame decisions (via `[CTA·Y/N]`) for Human validation. **→ Pushed PR #5 to `the-dyad-practice`.**
- **SUBMITTED (FOUNDING-GATED):** the **Proposal-Framing Playbook**.
  Node N4 (the PFP proposal artifact) has been successfully materialized in the Commons. We aligned the `playbook.md` nomenclature, purged the legacy `discipline.md` artifact, and bundled the proposal with the 5-testimonial single-dyad honesty flag. **→ Opened PR #6 in `the-dyad-practice`.**
