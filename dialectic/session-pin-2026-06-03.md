...
dyad-krishna's private anchor). **Emits only on a NEWLY-appearing unreachable source** (set-grow via
`comm` on the per-source tokens), **silent when one clears** — so a transient blip (a one-poll network fail
on a reachable repo) surfaces once on appearance, not twice (appear + clear). A persistent source surfaces
once. Mechanics are the
