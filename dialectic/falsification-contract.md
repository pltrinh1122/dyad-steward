...
> Clean, primer-free spec — the artifact to attack. *(Working history + contest record + panel/
> independence accounting live in `falsification-contract-draft.md`, kept OUT so a responder reads the
> rules unanchored.)* v3 integrates the cross-model + corpus-lens contest panel.
