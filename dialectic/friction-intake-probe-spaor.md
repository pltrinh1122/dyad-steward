...
| **REFLECT** | a terminal node (DONE/FALSIFIED) | **(1) CLOSURE**: re-run the originating PROBE's `falsification_target` — it must now *refute* (friction gone); if it still holds, RE-OPEN (the trail failed despite shipping). **(2)** crystallized lesson → durable substrate. **(3)** any NEW friction → back to Sense (may spawn a follow-up PROBE) | imposes **outcome over output** — only the last node sees the *originating friction* AND the *aggregate* post-fix state (no single EXECUTE can, post fan-out); the only durable-write; ships no functional change |
